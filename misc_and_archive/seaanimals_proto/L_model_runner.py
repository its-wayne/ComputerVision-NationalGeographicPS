...
st.image(Path(__file__).parent / "natgeobanner.png", use_container_width=True)
...
def load_model(model_path: str, classes: tuple[str, ...]):
...
def process_video(
    ...
    model = load_model(model_path, tuple(aquatic_classes))
...
uploaded_model = st.sidebar.file_uploader(
    "Custom model (.pt)", type=["pt"],
    help="Upload a fine-tuned YOLO-World .pt file. Leave blank to use the default yolov8x-worldv2.pt."
)

if uploaded_model is not None:
    _model_tmp = Path(tempfile.gettempdir()) / uploaded_model.name
    _model_tmp.write_bytes(uploaded_model.getbuffer())
    active_model_path = str(_model_tmp)
else:
    active_model_path = "yolov8x-worldv2.pt"
...
st.caption("YOLO-World + ByteTrack · upload one or more videos → tracked video + bounding box CSV + creature crops")
...
if uploaded_files:
    if not aquatic_classes:
        ...
        all_results: list[tuple[str, str, str]] = []  # (original_name, tracked_path, csv_path)

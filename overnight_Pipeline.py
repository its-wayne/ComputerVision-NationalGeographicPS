...
REFRESH_TOKEN  = os.environ["DROPBOX_REFRESH_TOKEN"]
        ...
        # ADJUST MISSION PATH
MISSION_PATH = "/DOEX0096_Palau/PLW_dscm_33/Video"


    #  # ADJUST video selection OR  set to None for every video
SELECTED_VIDEOS = None


CHECKPOINT_PATH = Path(__file__).parent / "finetuning" / "checkpoints" / "best.pt"  # set to None to use pretrained weights

_MISSION_DIR      = Path(__file__).parent / "processed_missions" / "mission_33"
_MISSION_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_CSV_PATH   = _MISSION_DIR / "mission_33_detections.csv"
OUTPUT_CROPS_DIR  = _MISSION_DIR / "mission_33_crops"      # subfolders per video
OUTPUT_EMBEDDINGS = _MISSION_DIR / "mission_33_embeddings.json"

CONF_THRESHOLD  = 0.15   # change to 0.10 for more detections
IOU_THRESHOLD   = 0.70
MIN_TRACK_LEN   = 3      # discard tracks shorter than this (noise filter)
APPLY_CLAHE     = True
TRIM_BLACK      = True
PRERESIZE       = 1080   # resize video to this height before inference (None to skip)
CROPS_PER_TRACK = 3      # how many crops to extract per track (one per equal segment)
CROP_PAD        = 10     # padding in pixels around each bounding box
...
CROP_LABELS      = ["early", "mid", "late"]  # label suffix for each third
...
def get_embedding(embed_info, image_bgr: np.ndarray) -> list:
    kind, model, preprocess = embed_info
    if kind == "bioclip":
        from PIL import Image
        img = Image.fromarray(cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB))
        inp = preprocess(img).unsqueeze(0).to(INFERENCE_DEVICE)
        with torch.no_grad():
            vec = model.encode_image(inp).squeeze(0).cpu().numpy()
...
def process_video(
    video_path: Path,
    media_id: str,        # ← full filename WITH .mp4 (e.g. "PLW_dscm_05_c058.mp4")
    video_name: str,      # ← stem WITHOUT .mp4 (used for crop folder + embedding key)
    model,
    tracker_config: str,
    csv_path: Path,
    crops_dir: Path,
    ...
    """
    Run YOLO+ByteTrack on one video.
    - Appends ALL detections to csv_path (media_id = filename with .mp4 so it matches
      sentinel rows written in main, making resume reliable).
    - Filters tracks by MIN_TRACK_LEN
    - Extracts top-3 crops per track (best confidence from each third of detection span)
    - Saves crops to crops_dir / video_name /
 
    Returns {track_id: [crop_path, ...]} for the embedding step.
    """
    resized_path = None
    ...
    if PRERESIZE:
        tmp = Path(tempfile.gettempdir()) / f"{video_path.stem}_rs{PRERESIZE}.mp4"
    ...
    # Build median background model for motion gating
    N_BG = 20
    ...
    # Open CSV (append mode; write header only if file is new/empty)
    ...
    # Collect per-track detections: {track_id: [(frame_idx, conf, x1, y1, x2, y2, class_name)]}
    ...
    with tqdm(total=total_frames, desc=f"  YOLO {video_name}", unit="frame", leave=True) as pbar:
        while frame_idx < total_frames:
            ...
            # Motion gate: only run YOLO if there's foreground activity
            ...
            if has_activity:
                ...
                if results[0].boxes.id is not None:
                    ...
                    for box, tid, cid, conf in zip(boxes, track_ids, class_ids, confs):
                        ...
                        # consistently with sentinel rows written in main().
                        csv_writer.writerow([
    ...
    # For each track, split its detections into CROPS_PER_TRACK equal segments.
    # From each segment, pick the highest-confidence detection as the crop target.
    needed: dict = defaultdict(list)  # frame_idx -> [(tid, x1,y1,x2,y2,cls,label)]
    k = CROPS_PER_TRACK
    for tid, dets in long_tracks.items():
        ...
        for seg_i, label in enumerate(CROP_LABELS[:k]):
            ...
            if not segment:
                continue
            best = max(segment, key=lambda d: d[1])  # highest confidence
            fi, conf, x1, y1, x2, y2, cls_name = best
            needed[fi].append((tid, x1, y1, x2, y2, cls_name, label))
 
    # Second pass: seek to needed frames and save crops
    cap2 = cv2.VideoCapture(str(work_path))
    for fi in tqdm(sorted(needed.keys()), desc=f"  Cropping {video_name}", unit="frame", leave=False):
        # fi is relative to first_real (as stored during inference)
        cap2.set(cv2.CAP_PROP_POS_FRAMES, first_real + fi)
        ret, frame = cap2.read()
        if not ret:
            continue
        for tid, x1, y1, x2, y2, cls_name, label in needed[fi]:
            ...
            fname    = f"id{tid:04d}_{cls_slug}_f{first_real + fi:05d}_{label}.jpg"
...
def main():
    ...
    # Load any embeddings already saved (for resuming interrupted runs)
    all_embeddings: dict = {}
    ...
    with tempfile.TemporaryDirectory(prefix="natgeo_") as tmpdir:
        ...
        for entry in entries:
            ...
            try:
                ...
                # Embed each track's crops and store in JSON
                total_crops = sum(len(v) for v in track_crops.values())
                print(f"  Embedding {total_crops} crops across {len(track_crops)} tracks...")
                for tid, crop_paths in track_crops.items():
                    key    = f"{video_name}__{tid}"
                    images = [cv2.imread(p) for p in crop_paths]
                    images = [img for img in images if img is not None]
                    if not images:
                        continue
                    vecs    = [get_embedding(embed_info, img) for img in images]
                    mean_v  = np.mean(vecs, axis=0)
                    mean_v /= (np.linalg.norm(mean_v) + 1e-8)
                    all_embeddings[key] = mean_v.tolist()
 
                # Save embeddings after every video
                with open(OUTPUT_EMBEDDINGS, "w") as f:
                    json.dump(all_embeddings, f)
 
                # Write sentinel row if no detections so we can resume past this video.
                # Compares against media_id (= entry.name) which is now what
                # detection rows also use, so this check is reliable.
